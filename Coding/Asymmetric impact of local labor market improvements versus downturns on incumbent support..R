...
# 1. Extract coefficients and covariance matrix
...
p_asymmetric <- ggplot(pred_asym, aes(x = Delta_Unemployment_Rate, y = estimate)) +
  # 95% confidence interval ribbon (cluster-robust SEs)
  geom_ribbon(aes(ymin = conf_low, ymax = conf_high), fill = "#2c3e50", alpha = 0.2) +
  # Piecewise-linear asymmetric regression line
  ...
  # Rug plot showing the empirical data distribution
